...
df = (
    ...
    .option(
        "url",
        "jdbc:sqlserver://myserver.database.windows.net:1433;database=mydb;user=myuser;password=secret;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;",
    )
...
dfres.write.format("com.databricks.spark.sqldw").option(
    "url",
    "jdbc:sqlserver://myserver.database.windows.net:1433;database=mydb;user=myuser;password=secret;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;",
).option("forwardSparkAzureStorageCredentials", "true").option(
    "dbTable", "my_table_in_dw_copy"
).option(
    "tempDir", "wasbs://mycontainer@myaccount.blob.core.windows.net/"
).save()
